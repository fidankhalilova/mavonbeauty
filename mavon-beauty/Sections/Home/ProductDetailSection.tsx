...
import { ShoppingCart, Heart, Share2, Plus, Minus, Star, Facebook, Twitter } from 'lucide-react';
        ...
        <div className="max-w-7xl mx-auto px-4 py-9 mt-16">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        ...
                        >
                            {images.map((img, idx) => (
                                <button
                                    ...
                                    className={`w-32 h-32 shrink-0 overflow-hidden border-2 transition-all ${selectedImage === idx
                                            ? 'border-gray-800'
                ...
                <div className="space-y-6 w-100 ml-22.5">
                    {/* Brand */}
                    <p className="text-sm text-black uppercase tracking-wider">MAVON BEAUTY</p>
                    <h1 className="text-4xl font-bold text-gray-900 -mt-2.5">
                        Cetaphil Gentle Skin 
                    </h1>
                    <div className="text-3xl font-bold text-gray-900">$500.00</div>
                    <div className="flex items-center gap-2">
                        {[1, 2, 3, 4, 5].map((star) => (
                            <Star key={star} className="w-5 h-5 fill-[#0AA350] text-[#0AA350]" />
                        ))}
                        <span className="text-sm text-gray-500">(1)</span>
                    </div>
                    <div className="space-y-2">
                        <p className="text-[#0AA350] font-medium text-[14px]">Only 56 items in stock!</p>
                        <div className="w-full bg-gray-200 rounded-full h-2">
                            <div className="bg-[#0AA350] h-2 rounded-full" style={{ width: '45%' }}></div>
                        </div>
                    </div>
                    <div className="space-y-3">
                        <div className="flex items-center gap-2">
                            <span className="text-gray-700 font-semibold text-[14px]">Color:</span>
                            <span className="text-gray-500 text-[14px]">Peach orange</span>
                        </div>
                        <div className="flex gap-3">
                            {colors.map((color) => (
                                <button
                                    ...
                                    className={`w-8 h-8 rounded-lg border-2 transition-all ${selectedColor === color.id
                    ...
                    <div className="space-y-3">
                        <div className="flex items-center gap-2">
                            <span className="text-gray-700 font-semibold text-[14px]">Weight:</span>
                        ...
                        <div className="flex gap-3">
                            {weights.map((weight) => (
                                <button
                                    ...
                                    className={`px-3 py-1.25 rounded-[6px] border-2 font-medium transition-all ${selectedWeight === weight
